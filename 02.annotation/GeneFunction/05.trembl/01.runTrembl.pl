...
my $output_dir="01.pil_trembl_result";
...
foreach my $pep(@pep){
    $pep=~/\/([^\/]+)$/;
    my $output_id=$1;
    print O "/share/work/software/blast/ncbi-blast-2.2.31+/bin/blastp -query $pep -db pil_trembl -evalue 1e-5 -outfmt 6 -num_threads 10 -out $output_dir/$output_id.tab\n";
}
close O;
